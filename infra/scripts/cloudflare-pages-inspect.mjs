...
const deployments = await cfApi(`/accounts/${accountId}/pages/projects/${projectName}/deployments?per_page=10`);
const list = Array.isArray(deployments?.result) ? deployments.result : [];
result.deployment_count = list.length;
result.deployments = list.map((deployment) => ({
  id: deployment.id,
  ...
  url: deployment.url,
  source_branch: deployment.deployment_trigger?.metadata?.branch,
  source_commit_hash: deployment.deployment_trigger?.metadata?.commit_hash,
  source_commit_message: deployment.deployment_trigger?.metadata?.commit_message,
  is_skipped: deployment.is_skipped,
  latest_stage_name: deployment.latest_stage?.name,
  latest_stage_status: deployment.latest_stage?.status,
  latest_stage_started_on: deployment.latest_stage?.started_on,
  latest_stage_ended_on: deployment.latest_stage?.ended_on,
...
if (list[0]?.id) {
  const detail = await cfApi(`/accounts/${accountId}/pages/projects/${projectName}/deployments/${list[0].id}`);
  if (detail?.result) {
  ...
  const logs = await cfApi(`/accounts/${accountId}/pages/projects/${projectName}/deployments/${list[0].id}/history/logs`);
