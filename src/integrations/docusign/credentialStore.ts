...
const store = new Map<string, DocuSignCredential>();

function toPublic(credential: DocuSignCredential): DocuSignCredentialPublic {
  return {
    id: credential.id,
...
function upsertByUserAndAccount(credential: DocuSignCredential): void {
...
export const docuSignCredentialStore = {
  saveOAuth(params: {
    userId: string;
    accessToken: string;
    refreshToken?: string;
  ...
  getPublicByUser(userId: string): DocuSignCredentialPublic[] {
    return Array.from(store.values())
      .filter((credential) => credential.userId === userId)
      .map(toPublic);
  },

  getActiveByUser(userId: string): DocuSignCredential | null {
  ...
  decryptAccessToken(credential: DocuSignCredential): string {
    return decrypt(credential.tokenEncrypted);
  },

  decryptRefreshToken(credential: DocuSignCredential): string | null {
  ...
  rotateToken(params: {
    ...
    accountName?: string;
    baseUri?: string;
  }): DocuSignCredentialPublic | null {
    const existing = store.get(params.credentialId);
    if (!existing || existing.revokedAt) return null;

    const updated: DocuSignCredential = {
      ...existing,
      tokenEncrypted: encrypt(params.accessToken),
      tokenMasked: maskToken(params.accessToken),
      ...
      baseUri: params.baseUri ? normalizeBaseUri(params.baseUri) : existing.baseUri,
      metadata: {
        ...(existing.metadata ?? {}),
        ...(params.metadata ?? {}),
      },
