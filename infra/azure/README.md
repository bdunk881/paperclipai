...
- `production` no longer relies on kubelet identity access for Key Vault

This Terraform track now expects explicit workload managed-identity object IDs
for `Key Vault Secrets User` assignments so stale node-level grants are not
modeled in Terraform. The `infra-deploy.yml` workflow discovers the production
Container App principal ID when it exists and passes it into `terraform
plan/apply`.

The AKS kubelet identity must keep `AcrPull` only. Key Vault access belongs to
the active workload managed identity, not the node pool.
