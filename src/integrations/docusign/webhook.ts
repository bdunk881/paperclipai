...
export function verifyDocuSignWebhook(params: {
  rawBody: Buffer;
  ...
  if (!signatureHeader || !signatureHeader.trim()) {
    throw new ConnectorError("auth", "Missing DocuSign webhook signature header", 401);
  }
  ...
  if (replayCache.has(replayKey)) {
    throw new ConnectorError("auth", "DocuSign webhook replay detected", 409);
  }
  ...
  if (!valid) {
    throw new ConnectorError("auth", "Invalid DocuSign webhook signature", 401);
  }

  replayCache.set(replayKey, nowMs + FIVE_MINUTES_MS);
}

export function clearDocuSignWebhookReplayCache(): void {
  replayCache.clear();
}
