...
-- Supabase-managed mirror of migrations/024_hitl_activity_events.sql.
-- Renumbered + retimestamped during rebase: HEL-15 (canonical workflow runtime)
-- landed first. HEL-16 references the canonical `runs(id)` not legacy
-- `workflow_runs`.
    ...
    AND (
      ...
      OR EXISTS (
        ...
        WHERE runs.id = approvals.run_id
          AND runs.user_id = public.app_current_user_id()
      )
    )
  )
  WITH CHECK (
...
WHERE ticket_assignments.ticket_id = tickets.id
  ...
  AND EXISTS (SELECT 1 FROM public.agents WHERE agents.id = ticket_assignments.actor_id::uuid)
