...
export function verifyPostHogWebhook(params: {
  rawBody: Buffer;
  ...
  if (!signatureHeader || !signatureHeader.trim()) {
    throw new ConnectorError("auth", "Missing PostHog webhook signature header", 401);
  }
  ...
  if (replayCache.has(replayKey)) {
    throw new ConnectorError("auth", "PostHog webhook replay detected", 409);
  }
  ...
  if (!valid) {
    throw new ConnectorError("auth", "Invalid PostHog webhook signature", 401);
  }

  replayCache.set(replayKey, nowMs + FIVE_MINUTES_MS);
}

export function clearPostHogWebhookReplayCache(): void {
  replayCache.clear();
}
