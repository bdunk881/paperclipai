...
describe("buildTeamAssemblyPrompt", () => {
  ...
  it("omits the default role catalog when the request has no role library", () => {
  ...
  it("includes a supplied role library as optional reference material", () => {
    ...
    expect(prompt).toContain("Treat this as a glossary");
    expect(prompt).toContain("Own strategy, resource allocation");
  });
});
