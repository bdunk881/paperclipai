...
describe("settingsClient", () => {
  ...
  it("apiGet sends the user header without content-type and returns json", async () => {
    ...
    const result = await apiGet("/api/mcp/servers", { id: "user-1", email: "u@example.com", name: "User" });
  ...
  it("apiPost sends method, body, and content-type", async () => {
    ...
    const result = await apiPost("/api/mcp/servers", { name: "Linear MCP" }, null);
  ...
  it("apiDelete accepts 204 responses", async () => {
    ...
    await expect(apiDelete("/api/mcp/servers/server-1", null)).resolves.toBeUndefined();
  ...
  it("raises ApiError using payload messages when requests fail", async () => {
    ...
    await expect(apiGet("/api/protected", null)).rejects.toEqual(new ApiError("Access denied", 403));
  });
  ...
  it("falls back to status text when an error payload is missing", async () => {
    ...
    await expect(apiPost("/api/fail", {}, null)).rejects.toEqual(
