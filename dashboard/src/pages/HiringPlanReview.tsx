/**
 * Hiring Plan Review page (HEL-25).
 *
 * Side-by-side view: mission statement on the left, generated plan summary
 * on the right. The user reviews the plan and clicks Confirm to provision
 * agents + org_edges via POST /api/missions/:id/hiring-plans/:planId/confirm.
 */
...
export default function HiringPlanReview() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const data = await getHiringPlan(missionId, planId, token);
      setPlan(data);
  ...
  }, [missionId, planId, requireAccessToken]);
  ...
  async function handleConfirm() {
    ...
    try {
      const token = await requireAccessToken();
      await confirmHiringPlan(missionId, planId, token);
      setPageState("confirmed");
      // Refresh plan data so the confirmed state is reflected.
      const refreshed = await getHiringPlan(missionId, planId, token);
      setPlan(refreshed);
    } catch (err) {
