...
describe("PATCH /api/hiring-plans/:hiringPlanId/draft", () => {
  it("returns 401 when no authenticated user is present", async () => {
    const app = buildApp({ workspaceId: "11111111-1111-4111-8111-111111111111" });
    const res = await request(app)
      .patch("/api/hiring-plans/22222222-2222-4222-8222-222222222222/draft")
      .send({ includedRoleKeys: ["ceo"] });
    expect(res.status).toBe(401);
  });

  it("returns 400 when includedRoleKeys is not an array", async () => {
    const app = buildApp({
      sub: "user-1",
      workspaceId: "11111111-1111-4111-8111-111111111111",
    });
