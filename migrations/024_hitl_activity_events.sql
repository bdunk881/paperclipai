-- Migration 024: Canonical HITL approvals, tickets, and activity events (HEL-16)
--
-- Renumbered from 023 → 024 during rebase: HEL-15 (canonical workflow runtime)
-- landed first in the 023 slot. HEL-16 follows on top of the canonical model,
-- so `run_id` here references the canonical `runs(id)` (HEL-15) rather than
-- the now-dropped legacy `workflow_runs`.
--
-- Adds the canonical tables for human-in-the-loop gates and the customer-facing
-- activity stream while preserving the legacy tables that live code still uses.
-- Ticket SLA compatibility is intentional: existing priority/sla_state/due_date
-- columns and ticket_sla_* foreign keys remain in place.
...
CREATE INDEX IF NOT EXISTS idx_approvals_run_step
  ON approvals (run_id, step_id);

ALTER TABLE approvals ENABLE ROW LEVEL SECURITY;
ALTER TABLE approvals FORCE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS approvals_run_owner_or_assignee ON approvals;
CREATE POLICY approvals_run_owner_or_assignee
  ON approvals
  USING (
    app_current_user_id() IS NOT NULL
    AND (
      decided_by_user_id = app_current_user_id()
      OR payload ->> 'user_id' = app_current_user_id()
      OR payload ->> 'assignee' = app_current_user_id()
      OR EXISTS (
        SELECT 1
        FROM runs
        WHERE runs.id = approvals.run_id
          AND runs.user_id = app_current_user_id()
      )
    )
  )
  WITH CHECK (
...
CREATE OR REPLACE FUNCTION sync_ticket_body_description()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
...
DROP TRIGGER IF EXISTS tickets_sync_body_description ON tickets;
CREATE TRIGGER tickets_sync_body_description
  BEFORE INSERT OR UPDATE OF body, description ON tickets
  FOR EACH ROW
  EXECUTE FUNCTION sync_ticket_body_description();

CREATE INDEX IF NOT EXISTS idx_tickets_workspace_assigned_agent
  ON tickets (workspace_id, assigned_agent_id, created_at DESC)
  WHERE assigned_agent_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_tickets_workspace_assigned_user
  ON tickets (workspace_id, assigned_user_id, created_at DESC)
...
CREATE INDEX IF NOT EXISTS idx_activity_events_subject_gin
  ON activity_events USING gin (subject);
...
SELECT
  ...
  ticket_updates.created_at
FROM ticket_updates
JOIN tickets ON tickets.id = ticket_updates.ticket_id
ON CONFLICT (id) DO NOTHING;
...
JOIN LATERAL (
  ...
  FROM (SELECT 1) seed
  LEFT JOIN workspaces workspace_parent
  ...
  LEFT JOIN agent_teams team_parent
...
DROP POLICY IF EXISTS activity_events_no_update ON activity_events;
...
DROP POLICY IF EXISTS activity_events_no_delete ON activity_events;
