...
// Catalog slugs already represented by a bespoke live connector above — skip
// them in the catalog section so each integration appears exactly once.
// (teams' live connector key is "teams"; its catalog slug is "microsoft-teams".)
const LIVE_CONNECTOR_SLUGS = new Set([
  "slack",
  "gmail",
  "hubspot",
  "sentry",
...
/** "OAuth · API key" capability summary for a catalog entry. */
function authCapabilityText(entry: CatalogIntegration): string {
...
function IntegrationsPanel() {
  ...
  // Full backend catalog + the user's stored generic connections. These power
  // the rest of the integration catalog (SendGrid, Notion, Zendesk, …) beyond
  // the bespoke live connectors above.
  const [catalog, setCatalog] = useState<CatalogIntegration[]>([]);
  const [catalogConnections, setCatalogConnections] = useState<CatalogConnection[]>([]);
  const [catalogTarget, setCatalogTarget] = useState<CatalogIntegration | null>(null);

  const loadCatalog = useCallback(async () => {
    try {
      const [{ catalog: entries }, token] = await Promise.all([
        fetchIntegrationCatalog(),
        getAccessToken(),
      ]);
      setCatalog(entries.filter((entry) => !LIVE_CONNECTOR_SLUGS.has(entry.slug)));
      if (token) {
        setCatalogConnections(await listCatalogConnections(token));
      }
    } catch {
      // Non-fatal: the live connectors above still render.
    }
  }, [getAccessToken]);
  ...
  // Surface the result of an OAuth round-trip (the backend callback redirects
  // back here with ?status=success|error&provider=…&message=…), then scrub the
  ...
  // ----- Generic catalog connect/disconnect (api_key / bearer / basic + OAuth) -----

  const connectionForSlug = useCallback(
    (slug: string) => catalogConnections.find((conn) => conn.integrationSlug === slug),
    [catalogConnections],
  );

  async function submitCatalogCredentials(
    entry: CatalogIntegration,
    credentials: { token?: string; username?: string; password?: string; instanceDomain?: string },
  ) {
    setBusyKey(entry.slug);
    try {
      const token = await getAccessToken();
      if (!token) throw new Error("Not authenticated");
      await createCatalogConnection(token, {
      ...
      toast.error(err instanceof Error ? err.message : `Couldn't connect ${entry.name}`);
      throw err;
    } finally {
      setBusyKey(null);
    }
  ...
  async function startCatalogOAuthFlow(
    entry: CatalogIntegration,
    ...
    try {
      const token = await getAccessToken();
      if (!token) throw new Error("Not authenticated");
      const url = await startCatalogOAuth(token, entry.slug, params);
      window.location.assign(url);
    } catch (err) {
  ...
  async function disconnectCatalog(entry: CatalogIntegration) {
    const conn = connectionForSlug(entry.slug);
    if (!conn) return;
    if (!window.confirm(`Disconnect ${entry.name}? Agents will lose access until reconnected.`)) {
      return;
    }
    setBusyKey(entry.slug);
    try {
      const token = await getAccessToken();
      if (!token) throw new Error("Not authenticated");
      await deleteCatalogConnection(token, conn.id);
  ...
  function catalogActionButton(entry: CatalogIntegration): ReactNode {
    const isBusy = busyKey === entry.slug;
    const connected = Boolean(connectionForSlug(entry.slug));
    if (connected) {
      return (
        <button
          type="button"
  ...
  return (
    <div className="panel" role="tabpanel" id="con-int">
      ...
      {catalogTarget ? (
        <CatalogConnectModal
          entry={catalogTarget}
          busy={busyKey === catalogTarget.slug}
          onClose={() => setCatalogTarget(null)}
          onSubmitOAuth={(params) => void startCatalogOAuthFlow(catalogTarget, params)}
          onSubmitCredentials={(creds) => void submitCatalogCredentials(catalogTarget, creds)}
        />
      ) : null}
    </div>
  );
...
// ---------------------------------------------------------------------------
// Catalog connect modal — generic framework (OAuth BYO app + API key / basic).
// Unlike ConnectModal (live connectors with pre-registered OAuth apps), catalog
// integrations connect via the user's own OAuth app credentials or a static key.
// ---------------------------------------------------------------------------

interface CatalogConnectModalProps {
  entry: CatalogIntegration;
  busy: boolean;
  onClose: () => void;
  onSubmitOAuth: (params: {
    clientId: string;
    clientSecret?: string;
...
function CatalogConnectModal({
  entry,
  busy,
  onClose,
  onSubmitOAuth,
  onSubmitCredentials,
}: CatalogConnectModalProps) {
  const hasBoth = entry.supportsOAuth && entry.supportsApiKey;
  const [method, setMethod] = useState<"choose" | "oauth" | "api-key">(
    hasBoth ? "choose" : entry.supportsOAuth ? "oauth" : "api-key",
  );
  const isBasic = entry.authKind === "basic";
  ...
  const needsInstance = entry.requiresInstanceDomain;
  const instanceField = needsInstance ? (
    <label className="field">
      Instance domain
      <input
        type="text"
  ...
  const apiKeyValid = isBasic
    ? username.trim() && password.trim()
    : token.trim();
  const oauthValid = clientId.trim() && (!needsInstance || instanceDomain.trim());
  const apiKeyAllValid = apiKeyValid && (!needsInstance || instanceDomain.trim());
      ...
      <div className="af2-v2-modal" style={{ maxWidth: 480 }}>
        ...
        {method === "choose" ? (
          <>
            <div className="af2-v2-modal-body">
              <p className="desc" style={{ marginTop: 0 }}>
                How would you like to connect {entry.name}?
              </p>
              <div style={{ display: "grid", gap: 10, marginTop: 12 }}>
                <button
                  type="button"
                ...
                >
                  <div>
                    <div style={{ fontWeight: 500 }}>Connect with OAuth</div>
                    <div className="desc" style={{ marginTop: 2 }}>
                      Use your own OAuth app credentials. Recommended.
                    </div>
                  </div>
                </button>
                <button
            ...
            onSubmit={(e) => {
              ...
              if (oauthValid) {
                onSubmitOAuth({
                  ...
                  instanceDomain: instanceDomain.trim() || undefined,
                });
              }
            }}
          >
            <div className="af2-v2-modal-body">
              <p className="desc" style={{ marginTop: 0 }}>
                Register an OAuth app with {entry.name} and paste its credentials.{" "}
                {entry.docsUrl ? (
                  <a href={entry.docsUrl} target="_blank" rel="noreferrer noopener" className="link-clay">
                    Docs ↗
                  </a>
                ) : null}
              </p>
              ...
              <label className="field">
                Client secret <span className="desc">(optional for PKCE public clients)</span>
                <input
                  type="password"
                  value={clientSecret}
            ...
            <div className="af2-v2-modal-foot">
              {hasBoth ? (
                <button type="button" className="btn ghost" onClick={() => setMethod("choose")} disabled={busy}>
                  ← Back
                </button>
              ) : null}
              <button type="button" className="btn" onClick={onClose} disabled={busy}>
              ...
              <button type="submit" className="btn primary" disabled={busy || !oauthValid}>
          ...
          >
            <div className="af2-v2-modal-body">
              <p className="desc" style={{ marginTop: 0 }}>
                Paste your {entry.name} credentials.{" "}
                {entry.docsUrl ? (
                  <a href={entry.docsUrl} target="_blank" rel="noreferrer noopener" className="link-clay">
                    Docs ↗
                  </a>
            ...
            <div className="af2-v2-modal-foot">
              ...
              <button type="submit" className="btn primary" disabled={busy || !apiKeyAllValid}>
                {busy ? "Saving…" : "Connect"}
              </button>
            </div>
          </form>
