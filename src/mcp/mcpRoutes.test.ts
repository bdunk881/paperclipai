...
describe("DELETE /api/mcp/servers/:id", () => {
  it("returns 401 without Authorization header", async () => {
    const s = await mcpStore.add(USER, { name: "x", url: "https://x.com" });
    const res = await request(app).delete(`/api/mcp/servers/${s.id}`);
    expect(res.status).toBe(401);
  });

  it("deletes and returns 204", async () => {
    const s = await mcpStore.add(USER, { name: "del me", url: "https://del.com" });
    const res = await request(app).delete(`/api/mcp/servers/${s.id}`).set(H);
    expect(res.status).toBe(204);
    expect(await mcpStore.list(USER)).toHaveLength(0);
  });
  ...
  it("returns 404 when server belongs to another user", async () => {
    const s = await mcpStore.add("other-user", { name: "x", url: "https://x.com" });
    const res = await request(app).delete(`/api/mcp/servers/${s.id}`).set(H);
    expect(res.status).toBe(404);
  });
});
...
describe("GET /api/mcp/servers/:id/tools", () => {
  ...
  it("returns tools list on successful MCP response", async () => {
    const s = await mcpStore.add(USER, { name: "My MCP", url: "https://mcp.example.com" });
  ...
  it("includes auth header when server has auth credentials", async () => {
    const s = await mcpStore.add(USER, { name: "Secure", url: "https://mcp.example.com", authHeaderKey: "Authorization", authHeaderValue: "Bearer secret" });
