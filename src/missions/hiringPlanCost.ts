...
const RATES: Partial<Record<ProviderName, Record<string, RateEntry>>> = {
  openai: {
    ...
    "gpt-5.4-nano": { promptPer1k: 0.00005, completionPer1k: 0.0002 },
...
function tierFallback(model: string): RateEntry {
  ...
  if (lc.includes("gpt-4-turbo")) {
    return { promptPer1k: 0.015, completionPer1k: 0.075 };
  }
  if (lc.includes("gpt-5.5") || lc.includes("sonnet") || lc.includes("gpt-4o") || lc.includes("gpt-4")) {
    return { promptPer1k: 0.003, completionPer1k: 0.015 };
  }
  if (lc.includes("haiku") || lc.includes("nano") || lc.includes("mini") || lc.includes("3.5")) {
