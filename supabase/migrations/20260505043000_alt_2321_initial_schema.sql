-- ALT-2321 draft Supabase bootstrap migration
-- Generated from schema.sql on 2026-05-05.
-- Transformations applied:
--   * stripped pg_dump session setup and OWNER statements
--   * rewrote app_current_user_id/app_current_workspace_id to read Supabase JWT claims with legacy session-setting fallback
--   * preserved existing tables, indexes, triggers, functions, RLS enables, FORCE RLS directives, and policies as a first-pass draft
-- Review notes:
--   * apply against an empty Supabase project only
--   * observability partition maintenance still assumes a privileged role
--   * current user_id columns remain text; no auth.users foreign keys are introduced in this draft
--   * additional RLS coverage for non-public-facing tables is tracked in docs/alt-2321-supabase-schema-plan.md
-- Known follow-ups are documented in docs/alt-2321-supabase-schema-plan.md.
...
CREATE FUNCTION public.app_current_user_id() RETURNS text
  ...
  SELECT COALESCE(
    ...
    NULLIF(COALESCE(current_setting('request.jwt.claims', true), '{}')::jsonb ->> 'sub', '')
  )
$$;
...
CREATE FUNCTION public.app_current_workspace_id() RETURNS uuid
  ...
  WITH claims AS (
    ...
    SELECT NULLIF(current_setting('app.current_workspace_id', true), '') AS raw_value
    UNION ALL SELECT NULLIF(current_setting('request.jwt.claim.workspace_id', true), '')
    UNION ALL SELECT NULLIF(current_setting('request.jwt.claim.workspaceId', true), '')
    UNION ALL SELECT NULLIF(current_setting('request.jwt.claim.extension_workspace_id', true), '')
    UNION ALL SELECT NULLIF(current_setting('request.jwt.claim.extension_workspaceId', true), '')
    UNION ALL SELECT NULLIF((SELECT payload ->> 'workspace_id' FROM claims), '')
  ...
  FROM candidates
  WHERE raw_value ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
  LIMIT 1
$$;
