...
import { createStepResultRoutes } from "./stepResultRoutes";

const stubPool = { query: jest.fn() } as unknown as Parameters<typeof createStepResultRoutes>[0];
...
function buildApp(overrides: { sub?: string; workspaceId?: string } = {}): express.Express {
  ...
  app.use("/api/runs/:runId/step-results", createStepResultRoutes(stubPool));
  return app;
}

describe("GET /api/runs/:runId/step-results", () => {
  it("returns 401 when no authenticated user is present", async () => {
    const app = buildApp({ workspaceId: "11111111-1111-4111-8111-111111111111" });
    const res = await request(app).get("/api/runs/some-run-id/step-results");
