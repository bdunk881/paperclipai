...
import { createWakeEventRoutes } from "./wakeEventRoutes";

const stubPool = { query: jest.fn() } as unknown as Parameters<typeof createWakeEventRoutes>[0];
...
function buildApp(overrides: { sub?: string; workspaceId?: string } = {}): express.Express {
  ...
  app.use("/api/wake-events", createWakeEventRoutes(stubPool));
  return app;
}

describe("GET /api/wake-events", () => {
  it("returns 401 when no authenticated user is present", async () => {
    const app = buildApp({ workspaceId: "11111111-1111-4111-8111-111111111111" });
    const res = await request(app).get("/api/wake-events");
