 ...
 *
 * Modules are imported ONCE in beforeAll and reused (matching the pattern in
 * subscriptionStore.integration.test.ts).  Calling jest.resetModules() inside
 * each it() block would clear migrationPromise, causing re-application of
 * migrations — migration 001 has CREATE POLICY statements without IF NOT
 * EXISTS guards and would fail on a second apply.  It also leaks connection
 * pools, causing unexpected behaviour under tight connection limits.
 *
 * Tables covered:
...
describe("P1 table RLS integration (HEL-70)", () => {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  let pgPool: Pool;
  let withWorkspaceContextFn: typeof import("../middleware/workspaceContext").withWorkspaceContext;
  ...
  async function seedAll(
    pool: Pool,
    withWsCtx: typeof import("../middleware/workspaceContext").withWorkspaceContext,
  ) {
    wfA = randomUUID();
    wfB = randomUUID();
    wfvA = randomUUID();
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      withWorkspaceContextFn = workspaceContext.withWorkspaceContext;
  ...
  afterAll(async () => {
    ...
    if (pg) {
      await pg.closePostgresPoolForTests().catch((err: unknown) => {
        console.error("[HEL-70] pool close failed:", err);
      });
    }
  }, 30_000);
  ...
  afterEach(async () => {
    ...
    // Delete top-down in cascade order; workspace ON DELETE CASCADE handles
    // most children. queryPostgres runs as the superuser (autoflow), which
    // bypasses RLS and allows the CASCADE to reach child tables with FORCE RLS.
    await pg.queryPostgres(`DELETE FROM workspaces WHERE id IN ($1, $2)`, [
      workspaceA,
      workspaceB,
    ]);
    await pg.queryPostgres(`DELETE FROM user_profiles WHERE user_id IN ($1, $2)`, [
      userA,
      userB,
    ]);
  });
  ...
  it(
    "isolates workflows, workflow_versions, routines, runs, step_results across workspaces and denies NULL-context reads",
    async () => {
      if (!canRunIntegration) {
        return;
      }
      ...
      await withRlsEnforcedContext(pgPool, ctxA, async (client) => {
        expect(
          (await client.query(`SELECT id FROM workflows WHERE id = $1`, [wfA])).rowCount,
        ).toBe(1);
        expect(
          (await client.query(`SELECT id FROM workflows WHERE id = $1`, [wfB])).rowCount,
        ).toBe(0);
      });
      ...
      await withRlsEnforcedContext(pgPool, ctxA, async (client) => {
        expect(
          (await client.query(`SELECT id FROM workflow_versions WHERE id = $1`, [wfvA])).rowCount,
        ).toBe(1);
        expect(
          (await client.query(`SELECT id FROM workflow_versions WHERE id = $1`, [wfvB])).rowCount,
        ).toBe(0);
      });
      await withRlsEnforcedContext(pgPool, ctxB, async (client) => {
        expect(
      ...
      await withRlsEnforcedContext(pgPool, ctxA, async (client) => {
        expect(
          (await client.query(`SELECT id FROM routines WHERE id = $1`, [routineA])).rowCount,
        ).toBe(1);
        expect(
          (await client.query(`SELECT id FROM routines WHERE id = $1`, [routineB])).rowCount,
        ).toBe(0);
      });
      await withRlsEnforcedContext(pgPool, ctxB, async (client) => {
        expect(
      ...
      await withRlsEnforcedContext(pgPool, ctxA, async (client) => {
        expect(
          (await client.query(`SELECT id FROM runs WHERE id = $1`, [runA])).rowCount,
        ).toBe(1);
        expect(
          (await client.query(`SELECT id FROM runs WHERE id = $1`, [runB])).rowCount,
        ).toBe(0);
      });
      await withRlsEnforcedContext(pgPool, ctxB, async (client) => {
        expect(
      ...
      await withRlsEnforcedContext(pgPool, ctxA, async (client) => {
        expect(
          (await client.query(`SELECT id FROM step_results WHERE id = $1`, [stepA])).rowCount,
        ).toBe(1);
        expect(
          (await client.query(`SELECT id FROM step_results WHERE id = $1`, [stepB])).rowCount,
        ).toBe(0);
      });
      await withRlsEnforcedContext(pgPool, ctxB, async (client) => {
        expect(
  ...
  it(
    "isolates activity_events, connector_connections, budgets, subscriptions, entitlements across workspaces and denies NULL-context reads",
    async () => {
      if (!canRunIntegration) {
        return;
      }
      ...
      await withRlsEnforcedContext(pgPool, ctxA, async (client) => {
        expect(
          (await client.query(`SELECT id FROM activity_events WHERE id = $1`, [actEvtA])).rowCount,
        ).toBe(1);
        expect(
          (await client.query(`SELECT id FROM activity_events WHERE id = $1`, [actEvtB])).rowCount,
        ).toBe(0);
      });
      await withRlsEnforcedContext(pgPool, ctxB, async (client) => {
        expect(
      ...
      await withRlsEnforcedContext(pgPool, ctxA, async (client) => {
        expect(
          (await client.query(`SELECT id FROM connector_connections WHERE id = $1`, [connA]))
            .rowCount,
        ).toBe(1);
        expect(
          (await client.query(`SELECT id FROM connector_connections WHERE id = $1`, [connB]))
            .rowCount,
        ).toBe(0);
      });
      await withRlsEnforcedContext(pgPool, ctxB, async (client) => {
      ...
      await withRlsEnforcedContext(pgPool, ctxA, async (client) => {
        expect(
          (await client.query(`SELECT id FROM llm_credentials WHERE id = $1`, [llmCredIdA]))
            .rowCount,
        ).toBe(1);
        expect(
          (await client.query(`SELECT id FROM llm_credentials WHERE id = $1`, [llmCredIdB]))
            .rowCount,
        ).toBe(0);
      });
      await withRlsEnforcedContext(pgPool, ctxB, async (client) => {
      ...
      await withRlsEnforcedContext(pgPool, ctxA, async (client) => {
        expect(
          (await client.query(`SELECT id FROM budgets WHERE id = $1`, [budgetA])).rowCount,
        ).toBe(1);
        expect(
          (await client.query(`SELECT id FROM budgets WHERE id = $1`, [budgetB])).rowCount,
        ).toBe(0);
      });
      await withRlsEnforcedContext(pgPool, ctxB, async (client) => {
        expect(
      ...
      await withRlsEnforcedContext(pgPool, ctxA, async (client) => {
        expect(
          (await client.query(`SELECT id FROM subscriptions WHERE id = $1`, [subA])).rowCount,
        ).toBe(1);
        expect(
          (await client.query(`SELECT id FROM subscriptions WHERE id = $1`, [subB])).rowCount,
        ).toBe(0);
      });
      await withRlsEnforcedContext(pgPool, ctxB, async (client) => {
        expect(
      ...
      await withRlsEnforcedContext(pgPool, ctxA, async (client) => {
        expect(
          (
            await client.query(`SELECT workspace_id FROM entitlements WHERE workspace_id = $1`, [
              workspaceA,
            ])
          ).rowCount,
        ).toBe(1);
  ...
  it(
    "approvals: workspace policy (027) hides cross-tenant approvals; NULL context denies",
    async () => {
      if (!canRunIntegration) {
        return;
      }
      ...
      await withRlsEnforcedContext(pgPool, ctxA, async (client) => {
        expect(
          (await client.query(`SELECT id FROM approvals WHERE id = $1`, [approvalA])).rowCount,
        ).toBe(1);
        expect(
          (await client.query(`SELECT id FROM approvals WHERE id = $1`, [approvalB])).rowCount,
        ).toBe(0);
      });
  ...
  it(
    "audit_log: workspace isolation holds; RESTRICTIVE no-update and no-delete policies silently reject mutations",
    async () => {
      if (!canRunIntegration) {
        return;
      }
      ...
      await withRlsEnforcedContext(pgPool, ctxA, async (client) => {
        expect(
          (await client.query(`SELECT id FROM audit_log WHERE id = $1`, [auditA])).rowCount,
        ).toBe(1);
        expect(
          (await client.query(`SELECT id FROM audit_log WHERE id = $1`, [auditB])).rowCount,
        ).toBe(0);
      });
      await withRlsEnforcedContext(pgPool, ctxB, async (client) => {
        expect(
      ...
      await withRlsEnforcedContext(pgPool, ctxA, async (client) => {
        const deleteResult = await client.query(`DELETE FROM audit_log WHERE id = $1`, [auditA]);
